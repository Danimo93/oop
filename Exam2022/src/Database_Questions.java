...
public class Database_Questions {
    ...
    //Because the exam said "You shall insert Quiz questions into tables using JDBC connection." I am making this class just for this purpose,
    //and I wanted to split this from the main Database class so as to try and creat some form of order... we'll see how that works out in the end
    ...
    public void printArray() throws SQLException {
        ArrayList<BinaryQuiz> bin = BQ();

        for (BinaryQuiz b: bin) {
            System.out.println("Question: " + b.getQuestion());
            System.out.println("------------------\n");
        }
    }

    */
    //Even more testing, just wanted to se if the array made above actually work as intended, by printing out its contents
    //it did not at first, so yeah, testing is good

    /*
    public void printArray() throws SQLException {
        ArrayList<MultichoiceQuiz> multy = MQ();

        for (MultichoiceQuiz m: multy) {
            System.out.println("Question: " + m.getQuestion());
            System.out.println(m.getAnswerA() + m.getAnswerB() + m.getAnswerC() + m.getAnswerD());
            System.out.println(m.getCorrectAnswer());
            System.out.println("------------------\n");
        }
    }
